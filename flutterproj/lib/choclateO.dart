...
class ChoclateO extends StatefulWidget {
  @override
  _ChocolateOState createState() => _ChocolateOState();
}

class _ChocolateOState extends State<ChoclateO> {
  ...
  Future<void> getProductsByCategory(String category) async {
    ...
    print("Fetching products with URL: $url"); // Debug: URL checking
    try {
      ...
      print(
          'Response Status: ${response.statusCode}'); // Debug: Response status
      if (response.statusCode == 200) {
        final responseData = json.decode(response.body);
        print(
            'Products Loaded: ${responseData['products'].length}'); // Debug: Number of products
        setState(() {
          products = List<Map<String, dynamic>>.from(responseData['products']);
        });
      } else {
        print(
            'Failed to load products: ${response.body}'); // Debug: Error message
      }
    } catch (error) {
      print('Error fetching products: $error'); // Debug: Exception message
    }
  }
  ...
  void initState() {
    super.initState();
    getProductsByCategory('شوكولاته');
  }
